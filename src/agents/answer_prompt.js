...
export function buildAnswerUserText({ doc = '', review_template = '', question = '', category = null, summary = null, recent_messages = [], terminal_tail = '', action = false, precedents = '', doctrine = '', liveContext = '', tries = 0, factCheck = '', definition_of_done = '', citedSources = '' } = {}) {
  const evidence = {
    supervision_doc: doc || '',
  ...
  return head + '\n\n' + live + doct + pre + spec + behavior + gt + cited + 'CONTEXT_JSON:\n' + JSON.stringify(evidence).slice(0, MAX_CONTEXT_CHARS);
}
