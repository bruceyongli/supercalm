...
| S9 | Content-blind sends (2026-07-21 incident: KEEP_WORKING re-demanded already-shipped work; operator: "never done the check, just blindly sending") | A push (keep-working / unstick / challenge) must OBSERVE current reality before drafting and CITE it in the send; fresh progress falsifies the "you stopped" premise = no send; probe outage = fail-CLOSED | intents.js required `checked` param (structurally unrenderable without a citation) + supervisor realityCheck() freshness pre-gate at all five push sites | Lab 25 (fresh-progress / probe-outage / stale-with-citation), mutation-verified; intents suite blind-push refusals | keep-working pushes withheld on fresh progress; pushes without citation (must be 0) | ◐ |
