...
// into "story" events for the non-technical session view.  FIXPACK r3 — full replacement.
//
    ...
    if (j.type === 'session_meta') {
      ...
      if (t === 'message' && p.role === 'user') {
        ...
        if (exit != null && exit !== 0) {
          ...
          if (!benign) atoms.push({ ts, kind: 'fail', text: firstLines(outText, 2), exitCode: exit });
        }
      } else if (t === 'reasoning') {
      ...
      if (typeof c === 'string') {
        ...
        for (const part of c) {
          if (part.type === 'text' && part.text) {
            ...
            atoms.push({ ts, kind: 'fail', text: firstLines(textOf(part.content), 2), indent });
          }
        }
      }
