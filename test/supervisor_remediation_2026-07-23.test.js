...
const CFG_24 = { model: 'glm-5.2', mode: 'copilot', calibrated_escalation: true, decision_memory: false, goal_doubt: true, doc: '# Task\n\n## Goal\nStyle the session sidebar.\n\n## Hard rules\n- Never push unverified work as complete.\n' };
...
await check('24 autonomy: explicit Supervisor Autopilot decides the same reversible in-scope fork', async () => {
  const ask = 'Sidebar style: (a) flush full-height or (b) inset cards? Your call — say the word and I apply it.';
  const evd = { terminal_tail: 'Both styles are implemented behind a flag.\n(a) flush full-height  (b) inset cards\nYour call — say the word and I apply it.\n> ', recent_messages: [], git: {} };
  const ctx = makeCtx({ sid: 's_test_openesc_auto_' + Math.abs(now % 100000), session: { question: ask, summary: 'agent offers a reversible implementation fork', category: 'decision' } });

  await __lab.runAnswer(ctx, { ...CFG_24, mode: 'autopilot' }, evd, 'question', 0, SNAPSHOT(), 0);

  assert.equal(ctx._sends.length, 1, 'Autopilot owns the in-scope implementation choice and continues');
  assert.match(ctx._sends[0], /flush full-height/i);
  assert.equal(ctx._state().openEscalations?.length || 0, 0, 'the delegated implementation choice is not bound as an operator escalation');
});
