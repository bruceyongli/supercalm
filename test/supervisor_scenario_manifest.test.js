...
assert.equal(SUPERVISOR_SCENARIO_MANIFEST_VERSION, 'SGR-2026-07-29.2');
assert.equal(SUPERVISOR_SCENARIO_FAMILY_COUNT, 72);
assert.equal(SUPERVISOR_SCENARIO_CASE_COUNT, 79);
...
assert.equal(new Set(SUPERVISOR_SCENARIOS.map((scenario) => scenario.id)).size, 79);
...
console.log('supervisor_scenario_manifest.test ok (72 families / 79 cases / 2 mode responses)');
