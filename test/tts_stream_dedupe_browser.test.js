...
const server = createServer(async (req, res) => {
  ...
  res.end(`<!doctype html><script type="module">
    ...
    await player.speakSmart('A sufficiently long report. '.repeat(20), handle, { onSegment: (item) => segments.push(item) });
    window.__result = { segments, plays: window.__audioPlays };
  </script>`);
});
...
try {
  ...
  assert.equal(result.plays, 3, 'duplicate opening audio is never enqueued');
} finally {
  await browser.close();
