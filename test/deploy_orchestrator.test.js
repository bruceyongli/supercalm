...
//   project autonomous-deploy off → REJECTED (autopublish_off), gate never runs
...
// project_helpers auto_publish round-trip (guards the positional _upsert edit + the per-project switch)
const PH = await import('../src/project_helpers.js');
let h = PH.setHelpers(proj.id, { auto_publish: true, isolation: true });
assert.equal(h.auto_publish, 1, 'auto_publish persisted');
assert.equal(h.isolation, 1, 'isolation still persisted alongside');
assert.equal(PH.helperEnabled(proj.id, 'autoPublish'), true, 'helperEnabled(autoPublish) reads the column');
PH.setHelpers(proj.id, { auto_publish: false, isolation: false });
assert.equal(PH.helperEnabled(proj.id, 'autoPublish'), false, 'toggles back off');
...
// project autonomous-deploy OFF → dequeued + REJECTED (never deployed), gate never runs
const iOff = enq('off');
let offGate = 0; const offGateSpy = async (id, o) => { offGate++; return gateApprove(id, o); };
let r = await O.orchestrateOnce({ publishOn: () => false, blocks: notBlocked, isolation: isoOn, gate: offGateSpy, publish: publishOk });
assert.equal(r.result.stage, 'REJECTED', "project's autonomous deploy off → REJECTED");
assert.equal(r.result.failure_code, 'autopublish_off', 'failure_code autopublish_off');
assert.equal(offGate, 0, 'gate never ran when the project is not opted in');
clear(iOff.id);

// breaker blocks → REJECTED, gate never runs
const iBlk = enq('blk');
let gateRan = 0; const gateSpy = async (id, o) => { gateRan++; return gateApprove(id, o); };
r = await O.orchestrateOnce({ publishOn: () => true, blocks: blocked, isolation: isoOn, gate: gateSpy, publish: publishOk });
...
// isolation OFF → REJECTED isolation_off
const iIso = enq('iso');
r = await O.orchestrateOnce({ publishOn: () => true, blocks: notBlocked, isolation: isoOff, gate: gateApprove, publish: publishOk });
...
const iGr = enq('gr');
published = 0;
r = await O.orchestrateOnce({ publishOn: () => true, blocks: notBlocked, isolation: isoOn, gate: gateReject, publish: publishCount });
...
const iOk = enq('ok');
published = 0;
r = await O.orchestrateOnce({ publishOn: () => true, blocks: notBlocked, isolation: isoOn, gate: gateApprove, publish: publishCount });
...
r = await O.orchestrateOnce({ publishOn: () => true, blocks: notBlocked, isolation: isoOn, gate: gateApprove, publish: publishOk });
