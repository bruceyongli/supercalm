...
// answering supervision calls, so the chain is closed and only an explicit operator override opens it.
{
  ...
  // only model_scan.js applies it at boot. A model that exists solely in the live scan otherwise misses
  // ROUTES_BY_ID and routeForModel falls back to the default proxy, producing a verdict about a model
  // that was never actually called.
  ...
  assert.match(lab, /EXACT_MODEL \? \{ fallback_models: \[MODEL\] \}/,
    'an explicitly requested lab model becomes the entire chain — no silent fallback');
  assert.match(lab, /configured=\$\{MODEL\} requested=\$\{requested\} routed=\$\{route\.model/,
    'the lab fails closed before a mismatched route call');
  assert.match(lab, /res\.model !== MODEL/,
    'the lab also verifies the model identity returned by the provider');
}
