...
- Deploy with `bin/deploy [patch|minor|major|X.Y.Z]` (default **patch**): **bump the release version → push to origin (GitHub, with tags) → restart**. Git ONLY — no rsync. **Origin is `https://github.com/bruceyongli/supercalm.git`** (the public repo; the old `gitlab` remote is the retired pre-scrub lineage — never push there). `bin/deploy` **requires a clean tree** (only committed work ships — the auto-bump must not let uncommitted edits be silently skipped). `bin/deploy` also works **run on host itself** (dogfooding): it detects it's on the target (`~/aios` + the launchd service) and refreshes/restarts locally instead of ssh-ing.
- **GitHub push WORKS headless — do NOT report it as broken (fixed 2026-07-13).** The repo-local credential helper in `.git/config` supplies `GITHUB_PAT_AIOS` from `~/.dev.vars` at push time, and `bin/deploy` re-installs that helper automatically if it's ever missing. Plain `git push origin main --follow-tags` succeeds from any headless agent shell. If a push errors: (1) confirm the token line `GITHUB_PAT_AIOS=…` exists in `~/.dev.vars`, (2) run `bin/deploy` once (it self-heals the helper), (3) only escalate to the operator if the token itself is rejected (HTTP 401/403 from GitHub). Never conclude "agent shells can't push", never leave a release un-pushed, never ask the operator to push for you.
- **Release versioning (must-follow workflow).** Single source of truth = `package.json` `version`; **nothing else hardcodes it**. The server reads it once at boot (`src/config.js` `VERSION`) and serves it at `/api/version` (+ `/healthz` + `/api/state`); everything else derives at runtime. **`bin/version` is the ONLY thing that edits the version** — it bumps `package.json` + `package-lock.json`, commits `release: vX.Y.Z`, and annotates tag `vX.Y.Z`. `bin/deploy` calls it on **every** deploy, so deploying inherently increments + propagates the version everywhere. **Never hand-edit the version.** The UI shows a bottom-right **new-version toast** (`web/version-badge.js`; click → reload, shows the new version number) when an open browser polls `/api/version` (every 30s + on focus/visibility/online) and finds the server moved to a newer build after a deploy — no persistent version pill. It's wired into the single desktop SPA shell (`web/app.html`) and every intentional standalone surface via `<script type="module" src="version-badge.js">`; add that tag to any new standalone page. The toast is **release-channel aware**: `/api/version` returns `{version, channel}` (a release is `stable` only when `bin/release` blessed that exact version, writing `data/release_channel.json` → `config.js` `RELEASE_CHANNEL`; routine `bin/deploy` = `every`), and Settings → Preferences → **Release notifications** (`aios_release_notify`: Stable only [default] / Every release / Off) filters it. The post-upgrade Settings orientation follows that same channel preference, is persisted once per version, auto-dismisses after 8 seconds, and removes its DOM node/click handler on dismissal so it can never leave an invisible hitbox over the composer. **Full release system — versioning, the two deploy paths, channels, the headless PAT push, and the forthcoming autonomous integrate-&-deploy pipeline — is documented in [`docs/wiki/release-system.md`](docs/wiki/release-system.md)** (plan: `docs/specs/autonomous-deploy-plan.md`).
...
- **Vanilla Node, no framework.** `node:http` + a tiny router in `src/server.js`; `node:sqlite`
  ...
  `web-push` for notifications; Agent View's AG-UI wire adapter is repository-native.
