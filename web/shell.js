...
export async function openLaunch(opts = {}) {
  ...
  const mic = wireMic(q('#nl-mic'), q('#nl-task'), q('#nl-mic-status'), {
    hint: () => toolBtns.find((b) => b.classList.contains('on'))?.dataset.tool,
    stt: () => { const p = q('#nl-project')?.value; return p && p !== '__new' ? `&project=${encodeURIComponent(p)}` : ''; }, // ground the dictated task in the chosen project
  }); // speak the task; STT matches the selected agent
